...
public class BarricadosController1 {
        ...
        if (!gameController.barricadosEnemies1.isEmpty()) {
            for (int i = 0; i < gameController.barricadosEnemies1.size(); i++) {
                if (gameController.barricadosEnemies1.get(i).enemyTimer > 0) {
                    gameController.barricadosEnemies1.get(i).dx = -((gameController.barricadosEnemies1.get(i).x - gameController.ball.x) / Math.sqrt(Math.pow((gameController.barricadosEnemies1.get(i).x - gameController.ball.x), 2) + Math.pow((gameController.barricadosEnemies1.get(i).y - gameController.ball.y), 2))) * AllEnemies.BarricadosModel1.enemySpeed;
                    if (gameController.ball.y < gameController.barricadosEnemies1.get(i).y) {
                        gameController.barricadosEnemies1.get(i).dy = -Math.sqrt(Math.pow(AllEnemies.BarricadosModel1.enemySpeed, 2) - Math.pow(gameController.barricadosEnemies1.get(i).dx, 2));
                    } else {
                        gameController.barricadosEnemies1.get(i).dy = Math.sqrt(Math.pow(AllEnemies.BarricadosModel1.enemySpeed, 2) - Math.pow(gameController.barricadosEnemies1.get(i).dx, 2));
                    }
    ...
    public static void setTimerForBarricados1(AllEnemies.BarricadosModel1 barricados1, GameController gameController) {
